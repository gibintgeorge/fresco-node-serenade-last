...
module.exports = {
  ...
  create: function (req, res, next) {
    ...
    Cars.create(carData).exec(function (err, cars) {
      if (err) {
        console.error(err);
  ...
  deleteAll: function (req, res, next) {
    Cars.destroy().exec(function (err) {
      if (err) return next(err);
      return res.send();
    });
  },
  search: function (req, res, next) {
    ...
    if (req.param("limit") != undefined) query["limit"] = req.param("limit");
    if (req.param("skip") != undefined) query["skip"] = req.param("skip");
    if (req.param("sort") != undefined) query["sort"] = req.param("sort");

    Cars.find(query).exec(function (err, cars) {
      if (err) return res.badRequest(err);
      if (cars.length > 0) return res.json(cars);
      else return res.notFound(cars);
    });
  },
};
